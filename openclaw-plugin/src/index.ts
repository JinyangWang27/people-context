...
export function parseMcpToolResult(result: McpToolResult, toolName: string): unknown {
...
async function callMcpTool(
  ...
  try {
    ...
    return parseMcpToolResult(result as McpToolResult, name);
  } finally {
    if (connected) {
      await client.close();
    }
...
export default defineToolPlugin({
  ...
  tools: (tool) => [
    ...
    tool({
      ...
      parameters: Type.Object({
        ...
        purpose: Type.Optional(
          Type.String({
            description:
              "Why the context is needed, e.g. 'communication' or 'scheduling'.",
    ...
    tool({
      ...
      async execute({ person_id, situation }, config: Config, context) {
        return callMcpTool(
          config,
          ...
          Type.Array(
            Type.Object({
              ...
              kind: Type.Optional(
                Type.String({
                  description:
                    "nickname, native_script, transliteration, handle, former_name, or other.",
                }),
              ),
