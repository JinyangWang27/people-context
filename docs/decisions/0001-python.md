...
candidates were Python (with the official `mcp` SDK's high-level server API), TypeScript (which has first-class support
...
Use **Python ≥ 3.11**, with the official `mcp` Python SDK (`MCPServer`, formerly `FastMCP`) for the server,
`pydantic` (v2) for
...
- **Go or Rust** — would give stronger static typing, a single compiled binary, and no runtime dependency
  ...
  and MCP tool surface, and because the official `mcp` SDK's Python support (now via `MCPServer`) is the most
