...
To release a version the commits would not compute on their own, request it with a `Release-As: <version>`
footer on a commit on `main`; see [Requesting an explicit version](#requesting-an-explicit-version).
...
## Requesting an explicit version

Requesting a version is a **release step, not part of any feature pull request**. Do not rely on a
...
git commit --allow-empty -m "chore: request the 2.0.0 release" -m "Release-As: 2.0.0"
git show -s --format='%(trailers:key=Release-As)' HEAD   # must print: Release-As: 2.0.0
...
check; if it prints nothing, fix the message before pushing. If the trailer is missing, the release pull request
simply keeps proposing the Conventional-Commit version — a visible outcome in its title.

`tests/test_packaging_metadata.py` asserts that the root project version, `server.json.version`, the Registry
package's pinned `--from` requirement, `mcpb/manifest.json.version`, the `mcpb/pyproject.toml` dependency pin,
`people_context.__version__`, and the locked root project in `uv.lock` all carry the same value, whatever Release
Please writes. `mcpb/manifest.json.manifest_version` stays the MCPB **schema** version and is asserted separately.
The guarantees a major version makes binding are in [compatibility.md](compatibility.md).

Version domains that wrap or distribute the server stay explicit rather than accidentally synchronized. The
`.codex-plugin` manifest is coupled to the primary release and rewritten by Release Please; the
`.claude-plugin`, OpenClaw, and Obsidian packages carry their own versions and are bumped on their own policy.
