...
reported, and a future-dated interaction is never treated as stale. Rows are ordered never-contacted first, then
oldest interaction, name, and id; when more people qualify than `--limit`, the command says so.
