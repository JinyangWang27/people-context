...
- [x] **M29.1 — Amend and withdraw staged candidates**
  - **Scope:** Add `AmendStagedCandidate` and `WithdrawStagedCandidates` beside `ReviewImport`/`CommitImport` in
    ...
    prompts, and the packaged `people-context://guide` with the chat review loop. Amendment re-runs every staging input
    and batch rule, including `relationship_type` normalization, distinct relationship endpoints, and
...
- [x] **M30.2 — Batch review in the browser**
  - **Scope:** Depends on M29.1 and M29.2. Add one batch page listing candidates ordered by `ordinal`, with a checkbox
    ...
    commit result's committed, unresolved, and already-committed counts. The page passes M29.1's `batch_digest` as
    `expected_batch_digest` to `CommitImport` and `WithdrawStagedCandidates`, compared under an immediate write lock.
    After every mutation the page reloads its state from the server.
...
- [x] **M30.3 — Inline edit in the browser**
  ...
  - **Acceptance:** an invalid edit is refused per field; a valid edit is visible through `pctx import review`
    ...
    of another batch or the wrong type.
  - **Out:** remote or LAN access, any authentication scheme, HTTPS, multi-user operation, a background daemon,
...
These five planned PRs are additional to all historical and supplemental totals above. Internal dependencies are
M31.1 → M31.2 → M31.3 → M31.4 → M31.5. Reuse delivered capture, evidence, temporal reads, coaching, and staging;
no new browser capability is required. Nuwa informs grounded extraction; Luban informs baseline-led refinement.
