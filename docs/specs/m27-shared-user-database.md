...
**Pre-upgrade prerequisite:** before any client starts a version implementing M27, inventory the selected path
in every existing CLI/MCP agent launch environment using the old version's `pctx db-path -v` and its actual
arguments, environment, and config. A terminal shell's result does not inventory desktop/plugin environments.
Pin each existing client's selected absolute path with an existing explicit override before upgrading. If an
environment cannot be inventoried, keep that client stopped until its old path is identified and pinned; do not
treat it as a fresh installation. Keep path inventories local and do not publish private paths or environment
contents in diagnostics or issue reports.

Once all existing clients are accounted for, deliberately choose the store to share, retain isolated stores if
desired, or relocate with the SQLite-consistent procedure below. Preserve pins until each client has explicitly
transitioned to the chosen destination. An already-created `~/.pctx/people.db` is not evidence that another
client's old store was migrated. This prerequisite applies even when that new default already exists.

The runtime discovery guard below is limited to candidates visible in the current process environment. It cannot
discover arbitrary custom `XDG_DATA_HOME` or `OPENCLAW_WORKSPACE` values from other or previous agent environments.
No match is not proof that this user has no legacy stores; the pre-upgrade inventory is the cross-environment
protection. Document this limitation in release/upgrade and setup/plugin guidance before shipping. Do not add a
whole-filesystem scan, client-environment scraping, or a new persistent migration registry.

Never automatically move, copy, merge, or delete an old database. When no explicit argument, environment override,
or config-file path wins, and `~/.pctx/people.db` is absent, check all current-environment legacy candidates:
...
including an explicit choice of the new path. With no visible legacy file and no new default, runtime creation is
allowed; existing installations must first complete the prerequisite above. This is a documented operator
prerequisite, not an inventory the runtime can automatically verify.
...
current-behavior descriptions of the old default. Put the cross-environment inventory/pinning prerequisite ahead
of upgrade or first-start instructions, including desktop/plugin auto-update guidance. Document how to keep an
old store via an explicit override and how to relocate deliberately with all clients stopped and SQLite-consistent
backup/copy handling, preserving
...
- Two existing agent environments share a home but only one carries a custom `XDG_DATA_HOME` or
  `OPENCLAW_WORKSPACE`: reproduce that the other environment cannot discover that legacy store. Exercise the
  documented old-version inventory and explicit pinning procedure, then verify both clients retain their selected
  stores across upgrade, even if a new default has been created. Cover each variable separately. Do not assert
  that current-environment discovery alone detects the hidden store; test preservation through explicit pins.
...
- An existing new default wins despite legacy files. An explicit old or new path bypasses the guard. A workspace
  directory without a database does not block creation. Legacy checks cover all current-environment candidates,
  not just the first; diagnostics must not claim to have inventoried other agent environments.
