...
def test_the_remember_prompt_does_not_forbid_the_commit_its_review_loop_ends_in(tmp_path: Path) -> None:
    """Regression: an absolute "never call `commit_import`" contradicted the loop's final step.

    An agent following the prohibition literally could never commit a batch the user had
    explicitly accepted. The usage skill scopes the rule to automatic, speculative, or same-turn
    commits, and the prompt now says the same.
    """
    server = build_server(db_path=tmp_path / "remember-commit.db")

    async def flow(client: Client) -> str:
        prompt = await client.get_prompt("remember", {"statement": "Nadia moved to Lisbon"})
        return prompt.messages[0].content.text  # type: ignore[union-attr]

    body = _run(server, flow)

    assert "Never call `commit_import` yourself" not in body
    assert "Never call `commit_import` automatically, speculatively, or in the same turn as staging" in body
    assert "call `commit_import` only when the user accepts the revised batch itself" in body
