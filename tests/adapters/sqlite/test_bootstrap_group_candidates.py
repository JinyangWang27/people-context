...
def test_a_reviewable_batch_of_group_candidates_survives_export_and_restore(
    ...
    assert {"group", "membership"} <= exported
    ...
    assert {row.candidate["type"] for row in restored} == {"person", "group", "membership"}
...
def test_a_group_reference_survives_the_round_trip_pointing_at_the_same_row(runtime: ApplicationRuntime) -> None:
    ...
    referenced = {
        candidate["group_candidate_id"] for candidate in rows.values() if candidate["type"] == "membership"
    }
...
def test_forgetting_a_person_erases_their_placements_and_leaves_everyone_else_theirs(
    ...
    assert sorted(candidate["type"] for candidate in remaining.values()) == ["group", "membership", "person"]
